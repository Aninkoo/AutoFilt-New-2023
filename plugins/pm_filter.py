    ...
    if query.data.startswith("file"):
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("📞 𝖢𝗈𝗇𝗍𝖺𝖼𝗍", url=f"t.me/isaiminiprime_admin")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def advantage_spell_chok(client, msg):
    mv_id, mv_rqst, reqstr1 = msg.id, msg.text, msg.from_user.id if msg.from_user else 0
    reqstr = await client.get_users(reqstr1)
    ...
    gs_parsed = list(dict.fromkeys([re.sub(r'\b(\-([a-zA-Z-\s])\-\simdb|(\-\s)?imdb|(\-\s)?wikipedia|\(|\)|\-|reviews|full|all|episode(s)?|film|movie|series)',
                                          '', i, flags=re.IGNORECASE) for i in g_s]))

    if not gs_parsed:
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[InlineKeyboardButton("🔎 𝖦𝗈𝗈𝗀𝗅𝖾", url=f"https://www.google.com/search?q={reqst_gle}")]]
        if NO_RESULTS_MSG:
    ...
    movielist = list(dict.fromkeys([movie.get('title') for imdb_s in [await get_poster(mov.strip(), bulk=True) for mov in gs_parsed] if imdb_s for movie in imdb_s]))
    ...
    btn = [[InlineKeyboardButton(text=movie.strip(), callback_data=f"spolling#{user}#{k}#{key}]")] for k, movie in enumerate(movielist)]
    btn.append([InlineKeyboardButton(text="Close", callback_data="close_data")])
    spell_check_del = await msg.reply_photo(photo=SPELL_IMG, caption=script.CUDNT_FND.format(mv_rqst), reply_markup=InlineKeyboardMarkup(btn))
